...
#region Right Little
                        ...
                        for (int j = 0; j < countX; j++)
                        {
                            ...
                            if (GUI.Button(rect, icon, guiStyleIconButton))
                            {
                                ...
                                if (handPoseMode == HandPoseMode.Left)
                                    ...
                                    set.SetRight();
                                va.LoadPoseTemplate(set .poseTemplate, false, false, VeryAnimation.PoseTemplateFlags.Humanoid);
                                if (va.mirrorEnable)
                                {
                                    if (handPoseMode == HandPoseMode.Left)
                                        set.SetRight();
                            ...
                            if (iconShowName)
                            {
                                EditorGUI.DropShadowLabel(rect, va.handPoseSetList[index].poseTemplate.name, guiStyleNameLabel);
                            }
                        }
                        EditorGUILayout.EndHorizontal();
                    }
        ...
        private void UpdateHandPoseSetListReorderableList()
        {
            ...
            handPoseSetListReorderableList = new ReorderableList(va.handPoseSetList, typeof(PoseTemplate), true, false, true, true);
                            ...
                            GenericMenu menu = new GenericMenu();
                            {
                                ...
                                while (enu.MoveNext())
                                {
                                    ...
                                    menu.AddItem(new GUIContent(enu.Current.Key), false, () =>
                                    {
                                        ...
                                        {
                                            Undo.RecordObject(vae, "Template HandPose");
            ...
            handPoseSetListReorderableList.onAddDropdownCallback = (Rect buttonRect, ReorderableList list) =>
            {
                Action<bool> AddItem = (isRight) =>
                {
                    ...
                    va.SavePoseTemplate(poseTemplate, VeryAnimation.PoseTemplateFlags.Humanoid);
                    {
        ...
        private void UpdateHandPoseSetIcon()
        {
            ...
            humanPoseHandler.GetHumanPose(ref defaultHumanPose);
